...
class EnvView extends JPanel implements ObsVectListener, Observer {
	...
	EnvView(Env env) {
		...
		try {
			imgAreaBloqueada = createImage(
					(ImageProducer) (this.getClass().getResource("images/toolbar/area_bloqueada.gif")).getContent());
		} catch (Exception xEx) {
			xEx.printStackTrace();
		}
		...
		for (double x = 0; x < _env.getWidth(); x++)
			for (double y = 0; y < _env.getHeight(); y++) {
				...
				if (_env.isAreaBloqueada(p) != null) {
					...
					if (imgAreaBloqueada == null) {
						g.setColor(Color.darkGray);
						g.fillRect((int) (x * cw), (int) (y * ch),
								(int) cw, (int) ch);

					} else {
						g.drawImage(imgAreaBloqueada,
								(int) (x * cw + 1),
								(int) (y * ch + 1),
								(int) cw - 1,
								(int) ch - 1,
